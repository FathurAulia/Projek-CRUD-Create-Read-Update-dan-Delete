...
from BukuTelepon import Ui_MainWindow
import sys
...
global id, nama, nama_keluarga, kota, telepon, email
...
class Window(QtWidgets.QMainWindow):
    ...
    def btnReset(self):
        self.ui.txtID.clear()
        self.ui.txtINama.clear()
        self.ui.txtNamaKeluarga.clear()
    ...
    def ListOnClick(self): 
        ...
        self.ui.txtINama.setText(self.ui.tblDaftar.item(self.ui.tblDaftar.currentRow(), 1).text())
        self.ui.txtNamaKeluarga.setText(self.ui.tblDaftar.item(self.ui.tblDaftar.currentRow(), 2).text())
        self.ui.txtEmail.setText(self.ui.tblDaftar.item(self.ui.tblDaftar.currentRow(), 3).text())
    ...
    def btnSimpanClick(self): 
        ...
        nama = self.ui.txtINama.text()
        nama_keluarga = self.ui.txtNamaKeluarga.text()
        ...
        try:
            ...
            self.c.execute('INSERT INTO "Pengguna" VALUES (?,?,?,?,?,?)',(id,nama,nama_keluarga,kota,telepon,email))
            self.conn.commit()
            self.c.close()
            self.conn.close()
    ...
    def btnDaftarClick(self):  
        ...
        self.ui.tblDaftar.setHorizontalHeaderLabels(('ID','Nama','Nama Keluarga','Kota','Telepon','Email'))
    ...
    def btnPerbaruiClick(self):  
        ...
        try:
            self.conn = sql.connect("DaftarTelepon.db")
            self.c = self.conn.cursor()  
            self.c.execute("UPDATE Pengguna SET Nama = ?, Nama Keluarga = ?, Kota = ?, \
                Telepon = ?, Email = ? WHERE id = ?",(nama,nama_keluarga,kota,telepon,email,id))
            self.conn.commit()
            self.c.close()
            self.conn.close()
